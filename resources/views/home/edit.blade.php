...
@section('content')
    ...
    <div class="container">
        <div class="flex justify-between my-5">
            <h2 class="text-yellow-600 text-xl">Edit-{{$post->name}}</h2>
        ...
        <div>
            <form action="{{route('update',[$post->id])}}" method="post" enctype="multipart/form-data">
                ...
                <div class="flex flex-col gap-5">
                    ...
                    <label >Name</label>
                    <input type="text" placeholder="Name.." name="name" value="{{$post->name}}" >
                    @error('name')
                    <div class="text-red-600">{{ $message }}</div>
                    @enderror
                    ...
                    dark:focus:border-blue-500"  name="description" placeholder="Write your thoughts here...">{{$post->description}}</textarea>
